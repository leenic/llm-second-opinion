...
| **Denylist** | Regardless of roots: basenames matching `config*.json`, `.env*`, `*.pem`, `*.key`, `id_rsa*`, `*.p12`, `*.pfx`, and any dotfile ⇒ `invalid_input`; so is any file under a dot-directory below its root (`.git/`, `.claude/`) — components of the root itself are not checked (0.2.1 follow-up). Defense in depth — the roots do the real work. |
