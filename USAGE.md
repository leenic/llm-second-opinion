...
| `timeout` | The call hit `request_budget_seconds` (200s by default) and was cancelled. The reply carries `elapsed_ms` so you can see how close it got. Retry, lower `reasoning_effort`, pass a smaller `max_tokens`, or raise the budget — but keep it under the 240s cap Claude Desktop enforces, past which Desktop cancels and the result is lost entirely. |
